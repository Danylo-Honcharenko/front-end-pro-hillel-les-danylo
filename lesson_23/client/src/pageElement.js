...
export class PageElement {
    ...
    updateHTMLTodo = (todo) => {
        ...
        const htmlTodoIds = liList.map((li) => $(li).data("todoId"));

        if (todoIds.length > htmlTodoIds.length) {
            const newTodoItem = todo.filter((element) => htmlTodoIds.indexOf(element.id) === -1);
            const liElementList = this.getPreparedLiList(newTodoItem);
            liElementList.forEach((element) => $(".js--todos-wrapper").append(element));
        }

        if (todoIds.length < htmlTodoIds.length) {
            liList.filter((element) => todoIds.indexOf($(element).data("todoId")) === -1)
